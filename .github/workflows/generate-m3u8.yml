...
def parse_channel_info(html_content):
    ...
    try:
        # ========== 核心：适配目标页面的真实结构 ==========
        # 找到所有频道项的外层容器（实际页面的频道项class为 "col-md-3 col-sm-4 col-xs-6"）
        channel_items = soup.find_all("div", class_=re.compile(r"col-md-\d+ col-sm-\d+ col-xs-\d+"))
        print(f"找到 {len(channel_items)} 个频道项容器")
        ...
        for item in channel_items:
            ...
            group = group_tag.get_text(strip=True) if group_tag else "默认分组"
        ...
        print(f"✅ 成功解析 {len(channels)} 个有效频道")
    except Exception as e:
        print(f"解析频道信息失败：{e}")
        import traceback
